...
## ドット絵（Pixel Art）を本物らしく見せるコツ
AI（特にSDXL系）は描画能力が高すぎるため、高解像度で生成すると「ドットが細かくなりすぎてアニメ調」に見えることがあります。以下の設定を試すと、よりレトロな質感になります。

1.  **低解像度で生成する**:
    - 通常（1024px等）ではなく、あえて **256px 〜 512px** などの低い解像度で生成してください。
2.  **最近傍法（Nearest Neighbor）で拡大する**:
    - 生成された画像を拡大する際、滑らかにするフィルタ（Bilinear等）ではなく、**Nearest Neighbor** を使用するノードで拡大してください。
    - これにより、1つのドットが大きな四角形として維持され、ジャギーの効いたレトロな見た目が強調されます。
3.  **専用LoRAやフィルタの併用**:
    - プロンプトでの調整には限界があるため、ComfyUIで「Pixelate」ノードを通すなど、後処理フィルタを併用するとより確実です。
